...
- **Session Management**: Separate session management for auth server and client applications
...
The application acts as an **OAuth2 Authorization Server** that issues tokens to client applications. Client applications act as OAuth2 clients that request authorization on behalf of users.

### Participants

1. **User**: The end user accessing the application through their browser
2. **Client Application**: The OAuth2 client application requesting authorization on behalf of the user
...
**What Happens:**
- User navigates to the client application
- Client application detects the user is not authenticated
- Client application redirects user to the Authorization Server's authorization endpoint
...
**Database Activity:**
- **Table**: Client session tables (managed by client application)
  - A new session is created for the client with the PKCE verifier and state
  - Stores session information including OAuth2 state and PKCE verifier
...
- User approves access for the client application
...
- **Table**: `oauth2_authorization_consent` (if consent is required)
  ...
  - Note: May not be used if `requireAuthorizationConsent=false` in client configuration
...
- Redirects user back to client application with authorization code in URL

**Redirect Example:**
```
HTTP/1.1 302 Found
Location: <client_redirect_uri>?code=<authorization_code>&state=<state>
```
...
- **Table**: `oauth2_authorization`
  ...
  - Stores:
    ...
    - `registered_client_id`: The client's registered ID
    - `principal_name`: User's email or identifier
    - `authorization_grant_type`: 'authorization_code'
    - `authorized_scopes`: Requested scopes (e.g., 'openid profile email')
...
#### 6. Client Application Receives Authorization Code

**What Happens:**
- User's browser is redirected to client callback URL with authorization code
- Client application validates the state parameter to prevent CSRF attacks
- Client application prepares to exchange the code for tokens

**Database Activity:**
- **Table**: Client session tables (managed by client application)
  - Client retrieves session to validate state parameter
  - Retrieves stored state and PKCE code verifier

#### 7. Client Application Exchanges Code for Tokens

**What Happens:**
- Client application makes a backend-to-backend request to the token endpoint
- Request includes:
  - Authorization code
...
#### 8. Client Application Receives Tokens
...
**Database Activity:**
- **Table**: `oauth2_authorized_client` (if using Spring Security OAuth2 Client)
  - Client application may store the tokens for future use
  - Stores:
    - `client_registration_id`: Client registration identifier
    - `principal_name`: User identifier
    - `access_token_type`: 'Bearer'
    - `access_token_value`: Access token (encrypted)
    - `access_token_issued_at`: Timestamp
    - `access_token_expires_at`: Timestamp
    - `access_token_scopes`: Granted scopes
...
#### 9. Client Application Uses Access Token to Call Resource Servers

**What Happens:**
- Client application includes access token in Authorization header when calling backend APIs
...
**What Happens:**
- When access token expires (after 15 minutes), client application uses refresh token to obtain new tokens
- Client application makes request to token endpoint with refresh token
...
- **Table**: `oauth2_authorization`
  ...
  - Generates new refresh token (if `reuseRefreshTokens=false`)
  - Updates record with:
...
- **Table**: `oauth2_authorized_client` (if using Spring Security OAuth2 Client)
  - Client application updates stored tokens
...
#### Client Application Tables (if using Spring Security OAuth2 Client)

9. **oauth2_authorized_client**: Stores tokens that the client application has obtained from the auth server
10. Client session tables: Client applications typically maintain their own session management for storing OAuth2 state, PKCE verifiers, and authentication information
...
- Registered client configurations for OAuth2 client applications
